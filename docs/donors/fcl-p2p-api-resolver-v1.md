...
`fcl::plugins::p2p_api_resolver` is a plugin-level resolver for API-over-P2P
metadata. It composes through `p2p_node` and `fcl.api.transport`; it is not
...
- Resolver publishes actual product API routes through `p2p_node`.
