...
`fcl::plugins::secret::provider` is a neutral infrastructure plugin for
...
| FCL `signing::provider` | Local-only plugin, key ids, purpose allow-list, schema-owned secret config and output profile checks. | Product plugins parsing private keys, secret env vars or crypto profiles directly. | `secret::provider` follows the same official plugin shape and local API style. | Plugin descriptor, schema, purpose denial and package install tests. |

## Security Model

`secret::provider` does not make a compromised process safe. If a product daemon
