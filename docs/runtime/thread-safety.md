...
| `fcl::api::transport::client` | Owner-confined | One client owns one API stream, pending calls and serialized writes. It is not a shared concurrent object. | `test_fcl_api_transport` client concurrency tests through one coroutine owner |
| `fcl::api::transport::serve_session` | Strand-owned admission state | Accepted API stream slots and drain wakeups are serialized on one strand, so `max_concurrent_streams` is stable with multi-worker runtimes. | `test_fcl_api_transport api_transport_serve_session_serializes_admission_on_multi_worker_runtime` |
