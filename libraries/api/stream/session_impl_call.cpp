...
[[nodiscard]] std::exception_ptr cancelled_call_error(
   std::uint64_t id) {
   try {
      FORGE_THROW_EXCEPTION(forge::api::core::exceptions::cancelled,
                            "API stream call was cancelled",
...
[[nodiscard]] std::exception_ptr deadline_call_error(
   std::uint64_t id) {
   try {
      FORGE_THROW_EXCEPTION(forge::api::core::exceptions::deadline_exceeded,
                            "API stream call deadline exceeded",
                            forge::exceptions::ctx("call_id", id));
   } catch (...) {
      return std::current_exception();
   }
...
[[nodiscard]] forge::api::core::capability capability_for(
...
[[nodiscard]] forge::raw::unpack_limits payload_limits(
   std::size_t payload_size, std::uint32_t max_bytes) {
   const auto bounded = static_cast<std::uint32_t>(
      std::min<std::size_t>(payload_size, max_bytes));
...
forge::api::core::method_kind session::impl::method_kind_for(
   ...
   const auto* method = forge::api::core::find_method(*descriptor,
                                                      request.method);
   return method == nullptr ? forge::api::core::method_kind::unary
                            : method->kind;
}

std::shared_ptr<session::impl::call_state>
session::impl::make_remote_call(
   ...
   if (plan && plan->local != nullptr) {
      if (const auto* api = plan->local->describe(request.api)) {
         if (const auto* descriptor = forge::api::core::find_method(
                *api, request.method)) {
            call->descriptor = *descriptor;
         }
      }
   }
   const auto per_call_bytes =
      static_cast<std::size_t>(negotiated_limits.max_buffered_bytes);
   const auto make_pipe = [&] {
      return forge::api::core::detail::make_local_stream_pair(
         *strand, negotiated_limits.max_item_bytes,
...
std::shared_ptr<session::impl::call_state>
session::impl::reserve_local_call(
   ...
   const auto directions_match =
      (kind == forge::api::core::method_kind::unary && !input && !output) ||
      (kind == forge::api::core::method_kind::server_stream && !input &&
       output) ||
      (kind == forge::api::core::method_kind::client_stream && input &&
       !output) ||
      (kind == forge::api::core::method_kind::bidirectional_stream && input &&
       output);
   if (!directions_match) {
      FORGE_THROW_EXCEPTION(
   ...
   const auto side_limit = local_high_side
                              ? std::numeric_limits<std::uint64_t>::max()
                              : call_id_side_bit;
   const auto requested_id = value.id.value != 0 ? value.id.value
                                                  : request.id.value;
   if (requested_id != 0) {
      const auto requested_high_side =
         (requested_id & call_id_side_bit) != 0;
      if (requested_high_side != local_high_side ||
          requested_id < next_call_id || requested_id >= side_limit) {
         FORGE_THROW_EXCEPTION(
            forge::api::core::exceptions::protocol_error,
   ...
   if (calls.contains(request.id.value) ||
       tombstones.contains(request.id.value)) {
      FORGE_THROW_EXCEPTION(forge::api::core::exceptions::protocol_error,
                            "API stream call_id is reserved or stale");
   }
   ...
   const auto deadline = value.deadline.count() > 0 ? value.deadline
                                                    : settings.deadline;
   if (deadline.count() > 0) {
...
session::impl::async_call_on_strand(
   forge::api::core::frame request, forge::api::core::method_kind kind,
   std::shared_ptr<forge::api::core::detail::stream_endpoint> input,
   std::shared_ptr<forge::api::core::detail::stream_endpoint> output,
   call_options value,
   const forge::api::core::method_descriptor* descriptor) {
   auto call = reserve_local_call(request, kind, std::move(input),
                                  std::move(output), value, descriptor);
   try {
      ...
      const auto admitted_before = std::count_if(
      ...
      if (call->descriptor && call->descriptor->request_decoder) {
         call->descriptor->request_decoder(
            request.payload,
            payload_limits(request.payload.size(),
...
boost::asio::awaitable<void> session::impl::wait_for_terminal(
   ...
   while (calls.contains(call->id.value)) {
      ...
      if (cancellation.cancelled() != boost::asio::cancellation_type::none &&
          !call->done) {
         cancel_call(call, cancelled_call_error(call->id.value),
                     call->request_written);
      }
   }
}

boost::asio::awaitable<void> session::impl::wait_for_credit(
   ...
   while (true) {
      ...
      if (flow.transferred_items < flow.limit_items &&
          item_bytes <= flow.limit_bytes - flow.transferred_bytes) {
         co_return;
      }
      call->wake.expires_at(timer::time_point::max());
      auto error = boost::system::error_code{};
...
boost::asio::awaitable<void> session::impl::wait_for_outbound_capacity(
   const std::shared_ptr<call_state>& call, std::size_t item_bytes) {
   while (true) {
      if (call->done || call->error) {
         if (call->error) {
      ...
      if (outbound_buffered_items < outbound_item_limit() &&
          outbound_buffered_bytes <= byte_limit &&
   ...
   try {
      while (call->outbound && !call->outbound->ended) {
         ...
         if (flow.transferred_items ==
                std::numeric_limits<std::uint64_t>::max() ||
             item->size() > std::numeric_limits<std::uint64_t>::max() -
      ...
      if (!call->done) {
         cancel_call(call, std::current_exception(),
                     call->request_written || !call->local_origin);
      }
   }
   if (call->outbound) {
...
boost::asio::awaitable<void> session::impl::wait_for_outbound_pump(
   const std::shared_ptr<call_state>& call) {
...
boost::asio::awaitable<void> session::impl::run_remote_call(
   ...
   try {
      ...
      if (call->kind == forge::api::core::method_kind::unary) {
         ...
         response = co_await dispatcher->dispatch_stream(
            std::move(request),
...
void session::impl::start_deadline(
   const std::shared_ptr<call_state>& call,
   ...
   boost::asio::co_spawn(
      *strand,
      [self = shared_from_this(), call]() -> boost::asio::awaitable<void> {
         auto error = boost::system::error_code{};
         co_await call->deadline.async_wait(
            boost::asio::redirect_error(boost::asio::use_awaitable, error));
         if (!error && !call->done) {
            self->cancel_call(call, deadline_call_error(call->id.value),
                              call->request_written || !call->local_origin);
         }
      },
      boost::asio::detached);
...
void session::impl::cancel_call(
   const std::shared_ptr<call_state>& call, std::exception_ptr error,
   bool notify_peer) {
   if (call->done) {
      return;
   }
   call->done = true;
   call->error = error ? std::move(error)
   ...
   if (notify_peer && peer_hello_received &&
...
void session::impl::discard_inbound(
   const std::shared_ptr<call_state>& call) noexcept {
   ...
   try {
      flow.endpoint->fail(std::make_exception_ptr(
         forge::api::core::exceptions::cancelled{
            "API stream handler completed before its input direction"}));
   } catch (...) {
...
void session::impl::finish_call(
   ...
   if (call->writes_in_flight != 0 ||
       (call->outbound && !call->outbound->pump_done)) {
      return;
   }
...
void session::impl::remember_tombstone(
   const std::shared_ptr<call_state>& call) {
   ...
   if (!call->local_origin && call->inbound &&
       call->inbound->discarding && !call->inbound->ended) {
...
void session::impl::install_inbound_observer(
   ...
   call->inbound->endpoint->set_observer(
      [owner = weak_from_this(), id = call->id.value](
         forge::api::core::detail::stream_event event, std::size_t bytes) {
         if (auto self = owner.lock()) {
            const auto executor = self->current_strand();
            if (executor) {
               boost::asio::dispatch(
                  *executor, [self, id, event, bytes] {
                     self->on_inbound_event(id, event, bytes);
                  });
            }
         }
      });
...
void session::impl::on_inbound_event(
   std::uint64_t id, forge::api::core::detail::stream_event event,
   std::size_t bytes) {
...
std::uint64_t
session::impl::aggregate_outstanding_credit() const noexcept {
   auto total = std::uint64_t{0};
   for (const auto& [_, call] : calls) {
      if (call->inbound && !call->inbound->ended && !call->done &&
          call->inbound->limit_bytes >=
             call->inbound->transferred_bytes) {
         const auto value = call->inbound->limit_bytes -
                            call->inbound->transferred_bytes;
         total = value > std::numeric_limits<std::uint64_t>::max() - total
                    ? std::numeric_limits<std::uint64_t>::max()
                    : total + value;
...
void session::impl::replenish_inbound_credit() {
   ...
   auto used = outstanding > std::numeric_limits<std::uint64_t>::max() -
                                buffered
                  ? std::numeric_limits<std::uint64_t>::max()
   ...
   for (auto& [_, call] : calls) {
      ...
      auto& flow = *call->inbound;
      const auto consumed_items = flow.transferred_items -
                                  flow.buffered_items;
      const auto consumed_bytes = flow.transferred_bytes -
                                  flow.buffered_bytes;
      const auto item_window =
         std::uint64_t{negotiated_limits.initial_window_items};
      const auto byte_window =
         std::uint64_t{negotiated_limits.initial_window_bytes};
      const auto item_target =
         item_window > std::numeric_limits<std::uint64_t>::max() -
                          consumed_items
            ? std::numeric_limits<std::uint64_t>::max()
            : consumed_items + item_window;
      const auto byte_target =
         byte_window > std::numeric_limits<std::uint64_t>::max() -
                          consumed_bytes
            ? std::numeric_limits<std::uint64_t>::max()
            : consumed_bytes + byte_window;
      const auto available = used >= negotiated_limits.max_buffered_bytes
                                ? std::uint64_t{0}
                                : negotiated_limits.max_buffered_bytes - used;
      const auto desired = byte_target > flow.limit_bytes
                              ? byte_target - flow.limit_bytes
                              : std::uint64_t{0};
...
boost::asio::awaitable<void> session::impl::async_serve_on_strand() {
   if (!dispatcher) {
      FORGE_THROW_EXCEPTION(forge::api::core::exceptions::protocol_error,
                            "API stream session has no dispatcher");
   }
...
boost::asio::awaitable<void> session::impl::async_close_on_strand() {
   ...
   accepting = false;
   closing = true;
   const auto grace = peer_hello_received
                         ? std::chrono::milliseconds{
                              negotiated_limits.shutdown_grace_ms}
                         : settings.disconnect_grace;
   const auto grace_deadline = timer::clock_type::now() + grace;
   while (!calls.empty() && grace.count() > 0 &&
          timer::clock_type::now() < grace_deadline) {
      session_wake->expires_at(grace_deadline);
      auto error = boost::system::error_code{};
      co_await session_wake->async_wait(
         boost::asio::redirect_error(boost::asio::use_awaitable, error));
      if (!error) {
   ...
   const auto write_deadline = timer::clock_type::now() +
                               settings.control_timeout;
   while (!writer_idle() && timer::clock_type::now() < write_deadline) {
      session_wake->expires_at(write_deadline);
      auto error = boost::system::error_code{};
      co_await session_wake->async_wait(
         boost::asio::redirect_error(boost::asio::use_awaitable, error));
      if (!error) {
