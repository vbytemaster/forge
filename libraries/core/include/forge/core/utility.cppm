...
export module forge.core.utility;
...
template <typename T> struct deduce<T&> {
   typedef T type;
};
template <typename T> struct deduce<const T&> {
   typedef T type;
};
template <typename T> struct deduce<T&&> {
   typedef T type;
...
namespace detail {
template <typename T> forge::true_type is_class_helper(void (T::*)());
template <typename T> forge::false_type is_class_helper(...);

template <typename T, typename A, typename... Args> struct supports_allocator {
 public:
...
template <typename T, typename A, typename... Args>
auto construct_maybe_with_allocator(A&&, Args&&... args) -> std::enable_if_t<!supports_allocator<T, A>::value, T> {
...
template <typename T, typename A, typename... Args>
auto maybe_augment_constructor_arguments_with_allocator(A&&, std::tuple<Args&&...> args,
...
template <typename Container> void move_append(Container& dest, Container&& src) {
   if (src.empty()) {
      return;
   } else if (dest.empty()) {
      dest = std::move(src);
...
template <typename Container> void copy_append(Container& dest, const Container& src) {
   if (src.empty()) {
      return;
   } else {
...
template <typename Container> void deduplicate(Container& entries) {
   if (entries.size() > 1) {
 ...
 */
template <typename Signature> class optional_delegate;

template <typename R, typename... Args> class optional_delegate<R(Args...)> : private std::function<R(Args...)> {
 public:
...
// outside of namespace forge becuase of VC++ conflict with std::swap
export template <typename T> void forge_swap(T& a, T& b) {
   T tmp = std::move(a);
   a = std::move(b);
   b = std::move(tmp);
}
