...
[[nodiscard]] ctx_ptr make_context() {
   ...
   if (!context) {
      exceptions::raise(exceptions::code::backend_error, "failed to allocate AES context");
   }
   return context;
}
...
[[nodiscard]] int checked_update_size(std::size_t size, const char* label) {
   if (size > static_cast<std::size_t>(std::numeric_limits<int>::max())) {
      exceptions::raise(exceptions::code::invalid_options, std::string(label) + " is too large");
   }
   return static_cast<int>(size);
}

void require_size(std::size_t size, std::size_t expected, const char* message, exceptions::code kind) {
   if (size != expected) {
      exceptions::raise(kind, message);
   }
}

void require_gcm_nonce(std::span<const std::uint8_t> nonce) {
   require_size(nonce.size(), aes_gcm_nonce_size, "AES-256-GCM requires 12-byte nonce", exceptions::code::invalid_nonce);
}

void require_gcm_tag(std::span<const std::uint8_t> tag) {
   require_size(tag.size(), aes_gcm_tag_size, "AES-256-GCM requires 16-byte tag", exceptions::code::invalid_tag);
}

void require_cbc_iv(std::span<const std::uint8_t> iv) {
   require_size(iv.size(), aes_cbc_iv_size, "AES-256-CBC requires 16-byte IV", exceptions::code::invalid_nonce);
}

void require_sink(const aes_byte_sink& sink) {
   if (!sink) {
      exceptions::raise(exceptions::code::invalid_options, "AES streaming requires output sink");
   }
}
...
aes256_key make_aes256_key(std::span<const std::uint8_t> input) {
   require_size(input.size(), aes256_key_size, "AES-256 key requires 32 bytes", exceptions::code::invalid_key);
...
aes256_gcm_encoder::aes256_gcm_encoder(aes256_gcm_encoder_options options) : _impl(std::make_unique<impl>()) {
   ...
   if (EVP_EncryptInit_ex(_impl->context.get(), EVP_aes_256_gcm(), nullptr, nullptr, nullptr) != 1 ||
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to initialize AES-GCM encryption");
   }
   if (!_impl->aad.empty() && EVP_EncryptUpdate(_impl->context.get(), nullptr, &out_size, _impl->aad.data(),
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to apply AES-GCM AAD");
   }
}
...
void aes256_gcm_encoder::write(std::span<const std::uint8_t> data) {
   if (_impl->finalized) {
      exceptions::raise(exceptions::code::invalid_options, "cannot write to finalized AES-GCM encoder");
   }
   if (data.empty()) {
      return;
   }
   ...
   if (EVP_EncryptUpdate(_impl->context.get(), out.data(), &out_size, data.data(),
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to encrypt AES-GCM payload");
   }
...
aes256_gcm_authentication aes256_gcm_encoder::finalize() {
   if (_impl->finalized) {
      exceptions::raise(exceptions::code::invalid_options, "AES-GCM encoder already finalized");
   }
   ...
   if (EVP_EncryptFinal_ex(_impl->context.get(), out.data(), &out_size) != 1) {
      exceptions::raise(exceptions::code::backend_error, "failed to finalize AES-GCM encryption");
   }
   emit_chunk(_impl->sink, out, out_size);
   ...
   if (EVP_CIPHER_CTX_ctrl(_impl->context.get(), EVP_CTRL_GCM_GET_TAG, static_cast<int>(tag.size()), tag.data()) != 1) {
      exceptions::raise(exceptions::code::backend_error, "failed to read AES-GCM tag");
   }
...
aes256_gcm_decoder::aes256_gcm_decoder(aes256_gcm_decoder_options options) : _impl(std::make_unique<impl>()) {
   ...
   if (EVP_DecryptInit_ex(_impl->context.get(), EVP_aes_256_gcm(), nullptr, nullptr, nullptr) != 1 ||
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to initialize AES-GCM decryption");
   }
...
void aes256_gcm_decoder::write(std::span<const std::uint8_t> data) {
   if (_impl->finalized) {
      exceptions::raise(exceptions::code::invalid_options, "cannot write to finalized AES-GCM decoder");
   }
   if (data.empty()) {
      return;
   }
   ...
   if (EVP_DecryptUpdate(_impl->context.get(), out.data(), &out_size, data.data(),
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to decrypt AES-GCM payload");
   }
   emit_chunk(_impl->sink, out, out_size);
}
...
void aes256_gcm_decoder::finalize() {
   if (_impl->finalized) {
      exceptions::raise(exceptions::code::invalid_options, "AES-GCM decoder already finalized");
   }
   _impl->finalized = true;
   ...
   if (EVP_CIPHER_CTX_ctrl(_impl->context.get(), EVP_CTRL_GCM_SET_TAG, static_cast<int>(_impl->tag.size()),
      ...
      exceptions::raise(exceptions::code::authentication_failed, "AES-GCM authentication failed");
   }
   emit_chunk(_impl->sink, out, out_size);
}
...
aes256_gcm_ciphertext encrypt_aes256_gcm(const aes256_gcm_encrypt_request& request) {
   ...
   auto encoder = aes256_gcm_encoder{aes256_gcm_encoder_options{
       .key = request.key,
       .nonce = request.nonce,
       .aad = request.aad,
       .ciphertext_sink =
...
bytes decrypt_aes256_gcm(const aes256_gcm_decrypt_request& request) {
   ...
   auto decoder = aes256_gcm_decoder{aes256_gcm_decoder_options{
       ...
       .aad = request.aad,
       .plaintext_sink =
           [&](std::span<const std::uint8_t> chunk) { plaintext.insert(plaintext.end(), chunk.begin(), chunk.end()); },
   }};
...
aes256_cbc_ciphertext encrypt_aes256_cbc(const aes256_cbc_encrypt_request& request) {
   ...
   if (EVP_EncryptInit_ex(context.get(), EVP_aes_256_cbc(), nullptr, request.key.bytes.data(), request.iv.data()) !=
       1) {
      exceptions::raise(exceptions::code::backend_error, "failed to initialize AES-CBC encryption");
   }
   if (!request.plaintext.empty() &&
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to encrypt AES-CBC payload");
   }
   total_size = out_size;
   if (EVP_EncryptFinal_ex(context.get(), out.data() + total_size, &out_size) != 1) {
      exceptions::raise(exceptions::code::backend_error, "failed to finalize AES-CBC encryption");
   }
   total_size += out_size;
...
bytes decrypt_aes256_cbc(const aes256_cbc_decrypt_request& request) {
   ...
   if (EVP_DecryptInit_ex(context.get(), EVP_aes_256_cbc(), nullptr, request.key.bytes.data(),
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to initialize AES-CBC decryption");
   }
   if (!request.encrypted.ciphertext.empty() &&
      ...
      exceptions::raise(exceptions::code::backend_error, "failed to decrypt AES-CBC payload");
   }
   total_size = out_size;
   if (EVP_DecryptFinal_ex(context.get(), out.data() + total_size, &out_size) != 1) {
      exceptions::raise(exceptions::code::authentication_failed, "AES-CBC decryption failed");
   }
   total_size += out_size;
   out.resize(static_cast<std::size_t>(total_size));
   return out;
