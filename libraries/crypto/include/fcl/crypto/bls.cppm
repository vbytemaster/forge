...
namespace fcl::crypto::bls::detail {
...
template <typename DataType> struct checked_data {
   ...
   static auto calculate_checksum(const DataType& data) {
      auto encoder = fcl::crypto::ripemd160::encoder();
...
template <typename Container> std::string serialize_base64url(const Container& data) {
   ...
   return fcl::crypto::base64url_encode(packed.data(), packed.size());
}

} // namespace fcl::crypto::bls::detail

export namespace fcl::crypto::bls {
...
class public_key {
 public:
   ...
   template <typename T> friend T& operator<<(T& ds, const public_key& key) {
      fcl::raw::pack(ds, fcl::unsigned_int(static_cast<std::uint32_t>(sizeof(key._affine_non_montgomery_le))));
   ...
   template <typename T> friend T& operator>>(T& ds, public_key& key) {
      ...
      FCL_ASSERT(size.value == sizeof(key._affine_non_montgomery_le));
...
class signature {
 public:
   ...
   template <typename T> friend T& operator<<(T& ds, const signature& sig) {
      fcl::raw::pack(ds, fcl::unsigned_int(static_cast<std::uint32_t>(sizeof(sig._affine_non_montgomery_le))));
...
class aggregate_signature {
 public:
   ...
   template <typename T> friend T& operator<<(T& ds, const aggregate_signature& sig) {
      ...
      fcl::raw::pack(ds, fcl::unsigned_int(static_cast<std::uint32_t>(sizeof(affine_non_montgomery_le))));
