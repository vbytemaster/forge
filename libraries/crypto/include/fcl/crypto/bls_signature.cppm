...
export module fcl.crypto.bls_signature;
...
class bls_signature {
 public:
   ...
   const bls12_381::g2& jacobian_montgomery_le() const {
      return _jacobian_montgomery_le;
   }
   const std::array<uint8_t, 192>& affine_non_montgomery_le() const {
   ...
   template <typename T> friend T& operator<<(T& ds, const bls_signature& sig) {
      // Serialization as variable length array when it is stored as a fixed length array. This makes for easier
      // deserialization by external tools
      fcl::raw::pack(ds, fcl::unsigned_int(static_cast<uint32_t>(sizeof(sig._affine_non_montgomery_le))));
      ds.write(reinterpret_cast<const char*>(sig._affine_non_montgomery_le.data()),
               sizeof(sig._affine_non_montgomery_le));
      return ds;
   }

   // Keep explicit stream serialization so the layout matches bls_aggregate_signature.
   template <typename T> friend T& operator>>(T& ds, bls_signature& sig) {
      // Serialization as variable length array when it is stored as a fixed length array. This makes for easier
      // deserialization by external tools
      fcl::unsigned_int size;
      fcl::raw::unpack(ds, size);
      FCL_ASSERT(size.value == sizeof(sig._affine_non_montgomery_le));
...
class bls_aggregate_signature {
 public:
   ...
   explicit bls_aggregate_signature(const bls_signature& sig) : _jacobian_montgomery_le(sig.jacobian_montgomery_le()) {}
   ...
   template <typename T> friend T& operator<<(T& ds, const bls_aggregate_signature& sig) {
      ...
      fcl::raw::pack(ds, fcl::unsigned_int(static_cast<uint32_t>(sizeof(affine_non_montgomery_le))));
      ds.write(reinterpret_cast<const char*>(affine_non_montgomery_le.data()), sizeof(affine_non_montgomery_le));
      return ds;
   }

   // Keep explicit stream serialization so the layout matches bls_signature.
   template <typename T> friend T& operator>>(T& ds, bls_aggregate_signature& sig) {
      // Serialization as variable length array when it is stored as a fixed length array.
      // This makes for easier deserialization by external tools
      fcl::unsigned_int size;
      fcl::raw::unpack(ds, size);
      std::array<uint8_t, 192> affine_non_montgomery_le;
      FCL_ASSERT(size.value == sizeof(affine_non_montgomery_le));
