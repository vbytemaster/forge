...
template <typename T> struct eq_comparator {
   static bool apply(const T& a, const T& b) {
      return a.serialize() == b.serialize();
   }
};

template <typename... Ts> struct eq_comparator<std::variant<Ts...>> {
   using variant_type = std::variant<Ts...>;
   struct visitor : public fcl::visitor<bool> {
      visitor(const variant_type& b) : _b(b) {}

      template <typename KeyType> bool operator()(const KeyType& a) const {
...
template <typename... Ts> struct less_comparator<std::variant<Ts...>> {
   using variant_type = std::variant<Ts...>;
   struct visitor : public fcl::visitor<bool> {
      visitor(const variant_type& b) : _b(b) {}
...
template <typename Data> struct shim {
   using data_type = Data;

   shim() {}

   shim(data_type&& data) : _data(std::move(data)) {}

   shim(const data_type& data) : _data(data) {}

   const data_type& serialize() const {
      return _data;
   }

   template <typename Stream> friend Stream& operator<<(Stream& s, const shim& value) {
      fcl::raw::pack(s, value._data);
      return s;
   }

   template <typename Stream> friend Stream& operator>>(Stream& s, shim& value) {
