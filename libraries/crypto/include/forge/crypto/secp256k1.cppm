...
export module forge.crypto.secp256k1;

import forge.crypto.bigint;
...
export namespace forge::crypto::secp256k1 {
...
FORGE_DECLARE_EXCEPTION_CATEGORY(code, "forge.crypto.secp256k1")
...
class public_key {
 public:
   ...
   ~public_key();
   //           bool verify( const forge::crypto::sha256& digest, const signature& sig );
   ...
   public_key(const compact_signature& c, const forge::crypto::sha256& digest, bool check_canonical = true);
...
class private_key {
 public:
   ...
   static private_key regenerate(const forge::crypto::sha256& secret);

   private_key child(const forge::crypto::sha256& offset) const;
    ...
    */
   forge::crypto::sha512 get_shared_secret(const public_key& pub) const;

   //           signature         sign( const forge::crypto::sha256& digest )const;
   compact_signature sign_digest(const forge::crypto::sha256& digest, bool require_canonical = true) const {
      return sign_compact(digest, require_canonical);
   }
   compact_signature sign_compact(const forge::crypto::sha256& digest, bool require_canonical = true) const;
   //           bool              verify( const forge::crypto::sha256& digest, const signature& sig );
...
using recover_bytes = forge::crypto::bytes;
