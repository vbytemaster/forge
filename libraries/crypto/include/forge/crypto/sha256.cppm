...
export module forge.crypto.sha256;
...
class sha256 : public add_packhash_to_hash<sha256> {
 public:
   ...
   uint32_t pop_count() const {
      return static_cast<uint32_t>(std::popcount(_hash[0]) + std::popcount(_hash[1]) + std::popcount(_hash[2]) +
...
inline size_t hash_value(const forge::crypto::sha256& s) {
   return s._hash[3];
}
...
template <> struct hash<forge::crypto::sha256> {
   size_t operator()(const forge::crypto::sha256& s) const {
      return *((size_t*)&s);
   }
};

inline std::ostream& operator<<(std::ostream& os, const forge::crypto::sha256& r) {
   os << "sha256(" << r.str() << ")";
   return os;
}
