...
export module forge.crypto.sha3;
...
class sha3 {
 public:
   ...
   static sha3 hash(const std::string& s, bool is_nist = true) {
      return hash(s.c_str(), s.size(), is_nist);
   }
   static sha3 hash(const sha3& s, bool is_nist = true) {
      return hash(s.data(), sizeof(s._hash), is_nist);
   }
   ...
   template <typename T> inline friend T& operator<<(T& ds, const sha3& ep) {
      ds.write(ep.data(), sizeof(ep));
      return ds;
   }

   template <typename T> inline friend T& operator>>(T& ds, sha3& ep) {
...
template <> struct hash<forge::crypto::sha3> {
   size_t operator()(const forge::crypto::sha3& s) const {
      return *((size_t*)&s);
   }
};
