...
recover_bytes recover(const recover_bytes& signature, const recover_bytes& digest) {
   ...
   if (context == nullptr) {
      FORGE_THROW_EXCEPTION(exceptions::backend_error, "failed to initialize secp256k1 recovery context");
   }

   if (signature.size() != 65 || digest.size() != 32) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_input, "invalid secp256k1 recovery input size");
   }

   int recid = signature[0];
   if (recid < 27 || recid >= 35)
      FORGE_THROW_EXCEPTION(exceptions::invalid_signature, "invalid secp256k1 recovery id");
   recid = (recid - 27) & 3;

   secp256k1_ecdsa_recoverable_signature sig;
   if (!secp256k1_ecdsa_recoverable_signature_parse_compact(context, &sig, (const unsigned char*)&signature[1],
                                                            recid)) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_signature, "invalid secp256k1 recoverable signature");
   }

   secp256k1_pubkey pub_key;
   if (!secp256k1_ecdsa_recover(context, &pub_key, &sig, (const unsigned char*)&digest[0])) {
      FORGE_THROW_EXCEPTION(exceptions::backend_error, "failed to recover secp256k1 public key");
   }
