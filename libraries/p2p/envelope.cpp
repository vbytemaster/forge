...
std::vector<std::uint8_t> signed_envelope::signing_payload(std::string_view domain) const {
   if (domain.empty()) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_options, "signed envelope domain must not be empty");
   }
   auto out = std::vector<std::uint8_t>{};
   auto append_len_bytes = [&out](std::span<const std::uint8_t> bytes) {
      auto length = forge::multiformats::varint_encode(bytes.size());
...
std::vector<std::uint8_t> signed_envelope::encode() const {
   if (signature.empty()) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_options, "signed envelope signature is empty");
   }
   auto out = std::vector<std::uint8_t>{};
...
void signed_envelope::verify(std::string_view domain, std::optional<peer_id> expected_signer) const {
   if (payload_type.empty() || payload.empty() || signature.empty()) {
      FORGE_THROW_EXCEPTION(exceptions::codec_error, "signed envelope is incomplete");
   }
   const auto actual_signer = signer();
   if (expected_signer && actual_signer != *expected_signer) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_identity, "signed envelope signer peer id mismatch");
   }
   const auto message = signing_payload(domain);
   if (!verify_identity_signature(key, message, signature)) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_identity, "signed envelope signature verification failed");
   }
}
...
signed_envelope signed_envelope::decode(std::span<const std::uint8_t> bytes) {
   ...
   while (!in.done()) {
      ...
      case 1:
         if (type != detail::wire_type::length_delimited) {
            FORGE_THROW_EXCEPTION(exceptions::codec_error, "signed envelope public key must be bytes");
         }
      ...
      case 2:
         if (type != detail::wire_type::length_delimited) {
            FORGE_THROW_EXCEPTION(exceptions::codec_error, "signed envelope payload type must be bytes");
         }
      ...
      case 3:
         if (type != detail::wire_type::length_delimited) {
            FORGE_THROW_EXCEPTION(exceptions::codec_error, "signed envelope payload must be bytes");
         }
      ...
      case 5:
         if (type != detail::wire_type::length_delimited) {
            FORGE_THROW_EXCEPTION(exceptions::codec_error, "signed envelope signature must be bytes");
         }
   ...
   if (!saw_key || !saw_payload_type || !saw_payload || !saw_signature) {
      FORGE_THROW_EXCEPTION(exceptions::codec_error, "signed envelope missing required fields");
   }
   return out;
}

signed_envelope signed_envelope::seal(const public_key& key, const forge::crypto::asymmetric::private_key& private_key,
                                      std::string_view domain,
                                      std::span<const std::uint8_t> payload_type,
                                      std::span<const std::uint8_t> payload) {
   if (payload_type.empty() || payload.empty()) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_options, "signed envelope payload and type must not be empty");
   }
   auto out = signed_envelope{
       .key = key,
