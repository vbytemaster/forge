 ...
 private:
   void validate_stream(const node::incoming_protocol_stream& stream) const {
      if (stream.protocol != protocol_) {
         FORGE_THROW_EXCEPTION(forge::p2p::exceptions::unsupported_protocol, "P2P API binding received wrong protocol",
      ...
      if (peer_policy_.require_known_peer) {
         if (owner_ == nullptr || !owner_->peers().find(stream.session.remote_peer).has_value()) {
            FORGE_THROW_EXCEPTION(forge::p2p::exceptions::peer_not_found, "P2P API peer is not known",
...
class api_builder {
 public:
   ...
   api_builder& protocol_id(std::string value) {
      protocol_ = forge::p2p::protocol_id{.value = std::move(value)};
      return *this;
   }

   api_builder& protocol_id(forge::p2p::protocol_id value) {
 ...
 private:
   node* owner_ = nullptr;
   forge::api::binding_plan plan_;
   forge::p2p::protocol_id protocol_{.value = "/forge/api/1"};
...
class route_builder {
 public:
   ...
   route_builder& protocol_id(forge::p2p::protocol_id value) {
      protocol_ = std::move(value);
      return *this;
   }
 ...
 private:
   forge::p2p::protocol_id protocol_{.value = "/forge/route/1"};
   node::protocol_handler handler_;
};
