...
endpoint parse_endpoint(std::string_view value) {
   if (!starts_with(value, scheme)) {
      exceptions::raise(exceptions::code::invalid_endpoint, "QUIC endpoint must use quic:// scheme");
   }

   value.remove_prefix(scheme.size());
   if (value.empty()) {
      exceptions::raise(exceptions::code::invalid_endpoint, "QUIC endpoint host is empty");
   }
   ...
   if (value.front() == '[') {
      ...
      if (end == std::string_view::npos || end + 2 > value.size() || value[end + 1] != ':') {
         exceptions::raise(exceptions::code::invalid_endpoint, "invalid bracketed QUIC endpoint");
      }
      ...
      if (separator == std::string_view::npos) {
         exceptions::raise(exceptions::code::invalid_endpoint, "QUIC endpoint port is missing");
      }
   ...
   if (host_value.empty() || port_value.empty()) {
      exceptions::raise(exceptions::code::invalid_endpoint, "QUIC endpoint host or port is empty");
   }
   ...
   if (result.ec != std::errc{} || result.ptr != last || parsed_port > std::numeric_limits<std::uint16_t>::max()) {
      exceptions::raise(exceptions::code::invalid_endpoint, "QUIC endpoint port is invalid");
   }
