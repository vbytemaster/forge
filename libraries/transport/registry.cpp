...
module forge.transport.registry;
...
void require_factory(bool valid) {
   if (!valid) {
      FORGE_THROW_EXCEPTION(exceptions::protocol_error, "transport registry requires non-empty factories");
   }
}
...
void registry::register_stream(endpoint::protocol_kind protocol, stream_connector_factory connector,
   ...
   if (impl_->streams.contains(protocol)) {
      FORGE_THROW_EXCEPTION(exceptions::duplicate_registration, "transport stream protocol already registered");
   }
...
void registry::register_session(endpoint::protocol_kind protocol, session_connector_factory connector,
   ...
   if (impl_->sessions.contains(protocol)) {
      FORGE_THROW_EXCEPTION(exceptions::duplicate_registration, "transport session protocol already registered");
   }
...
boost::asio::awaitable<stream_connection> registry::async_connect_stream(endpoint remote, connect_options options) {
   ...
   if (found == impl_->streams.end()) {
      FORGE_THROW_EXCEPTION(exceptions::unsupported_protocol, "no registered stream transport for endpoint protocol");
   }
...
boost::asio::awaitable<session_connection> registry::async_connect_session(endpoint remote, connect_options options) {
   ...
   if (found == impl_->sessions.end()) {
      FORGE_THROW_EXCEPTION(exceptions::unsupported_protocol, "no registered session transport for endpoint protocol");
   }
   auto connector = found->second.connector();
   co_return co_await connector.async_connect(std::move(remote), options);
}
...
boost::asio::awaitable<stream_listener> registry::async_listen_stream(endpoint local, listen_options options) {
   ...
   if (found == impl_->streams.end()) {
      FORGE_THROW_EXCEPTION(exceptions::unsupported_protocol, "no registered stream listener for endpoint protocol");
   }
...
boost::asio::awaitable<session_listener> registry::async_listen_session(endpoint local, listen_options options) {
   ...
   if (found == impl_->sessions.end()) {
      FORGE_THROW_EXCEPTION(exceptions::unsupported_protocol, "no registered session listener for endpoint protocol");
   }
   co_return co_await found->second.listener(std::move(local), options);
}
