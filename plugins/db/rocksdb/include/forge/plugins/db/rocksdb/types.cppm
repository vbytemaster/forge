...
namespace db_schema = ::forge::plugins::db::rocksdb;

export template <> struct forge::schema::rules<db_schema::config> {
   [[nodiscard]] static forge::schema::object_schema<db_schema::config> define() {
      auto schema = forge::schema::object<db_schema::config>();
      schema.field<&db_schema::config::path>("path")
         .required()
         .non_empty()
         .description("RocksDB TransactionDB path; required when the rocksdb plugin is enabled");
      schema.field<&db_schema::config::column_families>("column-families")
         .default_value(std::vector<std::string>{})
         .each_non_empty()
         .description("RocksDB column families to open in addition to default");
      schema.field<&db_schema::config::create_if_missing>("create-if-missing")
         .default_value(true)
         .description("Create the RocksDB database if it does not exist");
      schema.field<&db_schema::config::create_missing_column_families>(
