...
export module fcl.plugins.http.server.types;
...
export template <> struct fcl::schema::rules<fcl::plugins::http::server::config> {
   [[nodiscard]] static fcl::schema::object_schema<fcl::plugins::http::server::config> define() {
      auto schema = fcl::schema::object<fcl::plugins::http::server::config>();
      schema.field<&fcl::plugins::http::server::config::bind_address>("bind-address")
         .default_value("127.0.0.1")
         .description("HTTP server bind address");
      schema.field<&fcl::plugins::http::server::config::port>("port")
         .default_value(std::uint64_t{0})
         .range(0, 65'535)
         .description("HTTP server TCP port; 0 asks the OS to choose an available port");
      schema.field<&fcl::plugins::http::server::config::api_base_path>("api-base-path")
         .default_value("/")
         .description("Default base path for typed HTTP APIs");
      schema.field<&fcl::plugins::http::server::config::max_request_body_bytes>("max-request-body-bytes")
         .default_value(std::uint64_t{16 * 1024 * 1024})
         .range(1, 1024ULL * 1024ULL * 1024ULL);
      schema.field<&fcl::plugins::http::server::config::max_header_bytes>("max-header-bytes")
         .default_value(std::uint64_t{64 * 1024})
         .range(1, 16ULL * 1024ULL * 1024ULL);
      schema.field<&fcl::plugins::http::server::config::read_timeout_ms>("read-timeout-ms")
         .default_value(std::uint64_t{30'000})
         .range(1, 86'400'000);
      schema.field<&fcl::plugins::http::server::config::idle_timeout_ms>("idle-timeout-ms")
