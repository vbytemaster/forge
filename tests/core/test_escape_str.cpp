...
BOOST_AUTO_TEST_CASE(escape_control_chars) try {
   ...
   escaped_str = "\\u0008\\u000c\n"
                 ...
                 "\\u000b\\u000c\r\\u000e\\u000f\\u0010\\u0011\\u0012\\u0013\\u0014\\u0015\\u0016\\u0017\\u0018\\u0019"
                 "\\u001a\\u001b\\u001c\\u001d\\u001e\\u001f";
   BOOST_CHECK_EQUAL(escape_str(input, fcl::escape_control_chars::off).first, escaped_str);
}
FCL_LOG_AND_RETHROW();

BOOST_AUTO_TEST_CASE(empty) try {
   std::string input;
   BOOST_CHECK_EQUAL(escape_str(input, fcl::escape_control_chars::on, 256, "").first, "");

   input = "";
   BOOST_CHECK_EQUAL(escape_str(input, fcl::escape_control_chars::off, 512, {}).first, "");
}
FCL_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE(truncate) try {
   ...
   BOOST_CHECK_EQUAL(escape_str(input, fcl::escape_control_chars::on, 256, "").first, repeat_256_chars);
   ...
   input = repeat_512_chars;
   BOOST_CHECK_EQUAL(escape_str(input, fcl::escape_control_chars::on, 256, "<-the end->").first,
...
BOOST_AUTO_TEST_CASE(modify) try {
   ...
   std::string input = repeat_512_chars;
   BOOST_CHECK(escape_str(input, fcl::escape_control_chars::on, 256, "").second);
   ...
   input = repeat_512_chars;
   BOOST_CHECK(!escape_str(input, fcl::escape_control_chars::on, 512).second);

   input = repeat_512_chars;
   BOOST_CHECK(!escape_str(input, fcl::escape_control_chars::on).second);

   input = repeat_512_chars;
   BOOST_CHECK(!escape_str(input, fcl::escape_control_chars::on, 1024).second);
