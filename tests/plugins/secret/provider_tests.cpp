...
namespace secret_provider = fcl::plugins::secret::provider;

namespace {

constexpr auto secret_provider_section = "plugins.secret.provider";
...
[[nodiscard]] fcl::config::document provider_config(std::vector<fcl::config::value> secrets) {
   ...
   document.set("plugins.secret.provider.secrets", fcl::config::value::array_type(secrets.begin(), secrets.end()));
   return document;
}
...
[[nodiscard]] fcl::api::handle<secret_provider::api> configured_api(fcl::asio::runtime& runtime,
   ...
   fcl::asio::blocking::run(runtime, plugin.configure(fcl::config::component_view{document, secret_provider_section}));
...
BOOST_AUTO_TEST_CASE(secret_provider_descriptor_redacts_config_and_keeps_api_local) try {
   ...
   BOOST_TEST(descriptor->section == secret_provider_section);
   ...
   const auto redacted = fcl::config::redact(
      ...
      registry);
   const auto* value = redacted.try_get("plugins.secret.provider.secrets");
...
BOOST_AUTO_TEST_CASE(secret_provider_rejects_default_payload_limits_above_aes_update_ceiling) try {
   ...
   plaintext_document.set("plugins.secret.provider.default-max-plaintext-bytes", fcl::config::value{too_large});
   ...
   BOOST_CHECK_THROW(fcl::asio::blocking::run(
                        ...
                        plaintext_plugin.configure(
                           fcl::config::component_view{plaintext_document, secret_provider_section})),
   ...
   ciphertext_document.set("plugins.secret.provider.default-max-ciphertext-bytes", fcl::config::value{too_large});
...
BOOST_AUTO_TEST_CASE(secret_provider_encrypt_aes_gcm_rejects_oversized_aad_before_crypto) try {
   ...
   auto document = provider_config({secret_entry("data-key",
                                                ...
                                                {"payload.encrypt"},
                                                {"encrypt_aes_gcm"})});
   document.set("plugins.secret.provider.default-max-aad-bytes", fcl::config::value{4U});
...
BOOST_AUTO_TEST_CASE(secret_provider_decrypt_aes_gcm_rejects_oversized_plaintext_before_crypto) try {
   ...
   auto document = provider_config({secret_entry("data-key",
                                                ...
                                                {"payload.decrypt"},
                                                {"decrypt_aes_gcm"})});
   document.set("plugins.secret.provider.default-max-plaintext-bytes", fcl::config::value{32U});
...
BOOST_AUTO_TEST_CASE(secret_provider_encrypted_file_plaintext_limit_is_size_limit_exceeded) try {
   ...
   auto document = provider_config({secret_entry("encrypted",
                                                ...
                                                {"get_bytes"},
                                                true)});
   document.set("plugins.secret.provider.default-max-plaintext-bytes", fcl::config::value{4U});

   auto plugin = secret_provider::plugin{};
   auto runtime = fcl::asio::runtime{};
   BOOST_CHECK_THROW(fcl::asio::blocking::run(
                        runtime, plugin.configure(fcl::config::component_view{document, secret_provider_section})),
                     secret_provider::exceptions::size_limit_exceeded);
}
FCL_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE(secret_provider_config_rejects_encrypted_file_scrypt_params_above_configured_ceilings) try {
   ...
   auto document = provider_config({secret_entry("encrypted",
                                                ...
                                                {"get_bytes"},
                                                true)});
   document.set("plugins.secret.provider.encrypted-file-max-scrypt-n", fcl::config::value{512U});
...
BOOST_AUTO_TEST_CASE(secret_provider_config_value_can_be_delivered_by_fcl_env_source_adapter) try {
   ...
   BOOST_TEST(help.find("plugins.secret.provider.secrets") == std::string::npos);
}
FCL_LOG_AND_RETHROW();
